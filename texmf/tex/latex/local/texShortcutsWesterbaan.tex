...
%% \abs*[\Big] manually sizes delimiters
\let\oldabs\abs
...
% Template for daily class exit slips
\newcommand{\className}{Default Class Name}
\newcommand{\exitSlipQuestionTemplate}[2][]{
  \vspace*{0.5in}
  \noindent Exit slip #1
  \hspace*{\stretch{1}} \className
  \hspace*{\stretch{1}} Name: \fillin[\hfill\textbf{KEY}\hfill][40mm]\\
  #2%
  \ifprintanswers\else\vspace*{\stretch{1}}\fi%
}
\newcommand{\exitSlip}[2][]{
  \exitSlipQuestionTemplate[#1]{#2}
  \ifprintanswers\else
    \exitSlipQuestionTemplate[#1]{#2}
  \fi%
  \pagebreak
...
\cs_new:Npn \TheSemester {
  ...
  \int_case:nnF { 1 }
    {
      { \int_compare_p:n { \l__shortcuts_md_int < 515 } }
        { \tl_set:Nn \l__shortcuts_sem_tl { Spring } }

      { \int_compare_p:n { \l__shortcuts_md_int < 625 } }
  ...
  \tl_use:N \l__shortcuts_sem_tl \xspace \the\year \xspace
}
\ExplSyntaxOff
...
\newcommand*{\dnarr}[1]{\draw[->] ($(#1)+(-0.15,0.15)$)--($(#1)+(0.15,-0.15)$);}
...
\newcommand*{\nindep}{\not\!\indep}
\newcommand*{\ninfty}{{-\infty}}
\newcommand{\notrel}[3][R]{#2\,\slashed{#1}\,#3} %Denotes a relation for math125: xRy
%% Auto-size LR parens with optional manual sizing (\big,\bigg,etc)
\newcommand*{\parens}[2][]{{\left(\vphantom{#1[}#2\right)}}
\newcommand*{\OO}[1]{\mathcal{O}_{#1}}
\newcommand{\rel}[3][R]{#2\,#1\,#3} %Denotes a relation for math125: xRy
...
\newcommand*{\uparr}[1]{\draw[->] ($(#1)+(-0.15,-0.15)$)--($(#1)+(0.15,0.15)$);}
