...
\NewDocumentCommand \randInts { s o D<>{\randIntLower} D(){\randIntUpper} m O{1} D<>{} }{
  ...
  % #2 - [nz]: produce pm(magnitude), where magnitude in [min,max]
  % #3, #4 - <min> and (max) are *separate* optional delimited args; defaults are 1 and 5
  % #5 - clist of names; defines \name globally to the drawn integer
  % #6 - optional randInt multiplier t (defaults to 1)
  % #7 - optional clist of values to skip

  %set the upper and lower bounds for the random integers
  \int_set:Nn \l__lower_bnd { \int_min:nn {\fp_eval:n { round(#3) }}{\fp_eval:n { round(#4) }} }
  \int_set:Nn \l__upper_bnd { \int_max:nn {\fp_eval:n { round(#3) }}{\fp_eval:n { round(#4) }} }
  ...
  \clist_map_inline:nn {#7}{
    \tl_if_eq:nnTF {#2}{nz}{
      ...
      \fp_compare:nNnTF { abs(##1) } < { \l__lower_bnd * #6}{
        % out of range low - skip
        }{
          \fp_compare:nNnTF { abs(##1) } > { \l__upper_bnd * #6 }{
      ...
      \fp_compare:nNnTF
        ...
        { } % out of range - skip
        {
          \fp_compare:nNnTF { ##1 } > { \l__upper_bnd * #6 }{
            % out of range - skip
          }{
            \seq_put_right:Nx \l__used_seq { \fp_eval:n {##1} }
          }
            ...
            \fp_compare:nTF { \l_tmpa_fp == 1 }
              {
                ...
                \group_begin:
                  \ensuremath{
                    \bool_if:NTF \l_exam_print_sign_bool
                      { \num[group-separator={,}, group-digits=integer, explicit-sign=+]{
